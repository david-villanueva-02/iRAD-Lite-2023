...
class OffboardControl(Node):
    ...
    def timer_callback(self) -> None:
        ...
        XR = R*cos(self.angle*pi/180)
        YR = R*sin(self.angle*pi/180)
        ...
        if self.vehicle_status.nav_state == VehicleStatus.NAVIGATION_STATE_OFFBOARD:
            match self.mode:
                case 0:
                    self.publish_position_setpoint(R, 0.0, self.takeoff_height, 0.0)
                    if abs(X - R) < 0.5 and abs(Y) < 0.5 and abs(Z + 5) < 0.5:
                        self.get_logger().info("Making a circle!")
                        self.mode = 1
                case 1:
                    self.publish_position_setpoint(XR, YR, self.takeoff_height, 0.0)
                    if abs(X - XR) < 0.5 and abs(Y - YR) < 0.5 and abs(Z + 5) < 0.5:
                        self.angle += 15

                    if self.angle >= 360:
                        self.get_logger().info("A circle!")
                        self.mode = 2
                case 2:
                    self.land()
                    exit(0)
